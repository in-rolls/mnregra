...
def clean_and_save_csv(df, fn):
    ...
    if bfn.startswith('r6_state'):
        xc = 0
    elif bfn.startswith('r6_district'):
        xc = 1
    elif bfn.startswith('r6_block'):
        xc = 2
    elif bfn.startswith('r6_panchayat'):
        xc = 3
    xcols = list(df.columns[-xc:]) if xc != 0 else []
    new_labels = pd.MultiIndex.from_frame(df.iloc[:4].T.astype(str), names=['', '', '', ''])
    df = df.set_axis(new_labels, axis=1).iloc[4:]
...
if __name__ == '__main__':
    ...
    df = dfs[1]
    clean_and_save_csv(df, 'csv/r6_state.csv')
