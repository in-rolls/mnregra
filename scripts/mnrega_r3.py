...
if __name__ == '__main__':
    ...
    if False:
        r0 = s.get(f'https://nreganarep.nic.in/netnrega/demand_emp_demand.aspx?lflag=eng&file1=dmd&fin={year}-{year + 1}&fin_year={year}-{year + 1}&source=national&rblhpb=Both&Digest=eOiDx0ww9xhSKGK2Eqp8Zg', headers=headers)
        ...
        df = dfs[7]
        clean_and_save_csv(df, f'{year}-csv/r3_states.csv')
    ...
    for code in state_codes:
        ...
        states.append((name, f'demand_emp_demand.aspx?file1=dmd&page1=s&lflag=eng&state_name={name}&state_code={code}&fin={year}-{year + 1}&fin_year={year}-{year + 1}&source=national&rblhpb=Both&Digest=vnDOAvSzICKkkbRKsvG%2feA'))
    ...
    for state, href in states:
        ...
        try:
            ...
            df = dfs[7]
            df['state'] = state
            clean_and_save_csv(df, f'{year}-csv/r3_district_{state}.csv', 1)
            ...
            for dt in districts:
                ...
                if r3.status_code == 200:
                    try:
                        ...
                        clean_and_save_csv(df, f'{year}-csv/r3_block_{district}+{state}.csv', 2)
                        ...
                        for bl in blocks:
                            ...
                            if r4.status_code == 200:
                                try:
                                    ...
                                    df['district'] = district
                                    df['block'] = block
                                    clean_and_save_csv(df, f'{year}-csv/r3_panchayat_{block}+{district}+{state}.csv', 3)
                                except Exception as e:
                                    print('ERROR', e)
                            else:
