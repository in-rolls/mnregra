...
if __name__ == '__main__':
    ...
    if False:
        r0 = s.get(f'https://nreganarep.nic.in/netnrega/citizen_html/demregister.aspx?lflag=eng&fin_year={year}-{year + 1}&source=national&labels=labels&Digest=2sK2jsi9G7FHeqD/Cv4G1Q', headers=headers)
        ...
        df = dfs[3]
        clean_and_save_csv(df, f'{year}-csv/r5_1_states.csv')
        soup = BeautifulSoup(r0.content, 'lxml')
        states = []
        for l in soup.find_all('a', {'href': re.compile('demregister\.aspx.*')}):
            states.append((l.text, l['href']))

    states = []
    for code in state_codes:
        name = state_codes[code]
        states.append((name, f'demregister.aspx?lflag=eng&page=S&state_name={name}&state_code={code}&fin_year={year}-{year + 1}&source=national&Digest=uDPemZkY10B5eY/JhN5Sig'))

    for state, href in states:
        print('- State:', state)
        url = 'https://nreganarep.nic.in/netnrega/citizen_html/' + href
        print(url)
        try:
        ...
        try:
            ...
            df = dfs[3]
            df['state'] = state
            clean_and_save_csv(df, f'{year}-csv/r5_1_district_{state}.csv', 1)
            soup2 = BeautifulSoup(r1.content, 'lxml')
            districts = []
            for l in soup2.find_all('a', {'href': re.compile('demregister\.aspx.*')}):
            ...
            for dt in districts:
                ...
                if r3.status_code == 200:
                    try:
                        ...
                        clean_and_save_csv(df, f'{year}-csv/r5_1_block_{district}+{state}.csv', 2)
                        soup3 = BeautifulSoup(r3.content, 'lxml')
                        blocks = []
                        for l in soup3.find_all('a', {'href': re.compile('demregister\.aspx.*')}):
                        ...
                        for bl in blocks:
                            ...
                            if r4.status_code == 200:
                                try:
                                    ...
                                    df['district'] = district
                                    df['block'] = block
                                    clean_and_save_csv(df, f'{year}-csv/r5_1_panchayat_{block}+{district}+{state}.csv', 3)
                                except Exception as e:
                                    print('ERROR', e)
                            else:
