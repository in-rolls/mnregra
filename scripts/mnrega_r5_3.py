...
def clean_and_save_csv(df, fn, xc=0):
    ...
    df = df.set_axis(new_labels, axis=1).iloc[2:]
...
if __name__ == '__main__':
    ...
    if False:
        r0 = s.get(f'https://nreganarep.nic.in/netnrega/state_html/pmsr.aspx?lflag=eng&file1=dmd&fin={year}-{year + 1}&fin_year={year}-{year + 1}&source=national&Digest=mHtHUm456vl1V987GoOY6g', headers=headers)

        if r0.status_code != 200:
            print(f'Error to get start URL with status code: {r0.status_code}')
            sys.exit(-1)
        dfs = pd.read_html(r0.content, encoding='utf-8', flavor='html5lib')
        df = dfs[4]
        clean_and_save_csv(df, f'{year}-csv/r5_3_states.csv')
        soup = BeautifulSoup(r0.content, 'lxml')
        states = []
        for l in soup.find_all('a', {'href': re.compile('pmsr\.aspx.*')}):
            states.append((l.text, l['href']))
    states = []
    for code in state_codes:
        name = state_codes[code]
        states.append((name, f'pmsr.aspx?page=S&lflag=eng&state_name={name}&state_code={code}&fin_year={year}-{year + 1}&source=national&Digest=d4Jdel+2tOqn9bZL4YkpNQ'))
    ...
    for state, href in states:
        ...
        try:
            ...
            continue
        try:
            dfs = pd.read_html(r1.content, encoding='utf-8', flavor='html5lib')
            df = dfs[4]
            df['state'] = state
            clean_and_save_csv(df, f'{year}-csv/r5_3_district_{state}.csv', 1)
            soup2 = BeautifulSoup(r1.content, 'lxml')
            districts = []
            for l in soup2.find_all('a', {'href': re.compile('pmsr\.aspx.*')}):
            ...
            for dt in districts:
                ...
                if r3.status_code == 200:
                    try:
                        dfs = pd.read_html(r3.content, encoding='utf-8', flavor='html5lib')
                        df = dfs[4]
                        df['state'] = state
                        df['district'] = district
                        clean_and_save_csv(df, f'{year}-csv/r5_3_block_{district}+{state}.csv', 2)
                        soup3 = BeautifulSoup(r3.content, 'lxml')
                        blocks = []
                        for l in soup3.find_all('a', {'href': re.compile('pmsr\.aspx.*')}):
                        ...
                        for bl in blocks:
                            ...
                            if r4.status_code == 200:
                                try:
                                    dfs = pd.read_html(r4.content, encoding='utf-8', flavor='html5lib')
                                    df = dfs[4]
                                    df['state'] = state
                                    df['district'] = district
                                    df['block'] = block
                                    clean_and_save_csv(df, f'{year}-csv/r5_3_panchayat_{block}+{district}+{state}.csv', 3)
                                except Exception as e:
                                    print('ERROR', e)
                            else:
