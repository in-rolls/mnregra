...
def clean_and_save_csv(df, fn, xc=0):
    ...
    new_labels = pd.MultiIndex.from_frame(df.iloc[:4].T.astype(str), names=['', '', '', ''])
...
if __name__ == '__main__':
    ...
    if False:
        r0 = s.get(f'https://nreganarep.nic.in/netnrega/demand_emp_demand.aspx?lflag=eng&file1=empprov&fin={year}-{year + 1}&fin_year={year}-{year + 1}&source=national&rblhpb=Both&Digest=r0ZWa5ZLSVI6DGyJIMatKQ', headers=headers)
        ...
        df = dfs[7]
        clean_and_save_csv(df, f'{year}-csv/r5_4_states.csv')
        soup = BeautifulSoup(r0.content, 'lxml')
        states = []
        for l in soup.find_all('a', {'href': re.compile('demand_emp_demand\.aspx.*')}):
            states.append((l.text, l['href']))
    states = []
    for code in state_codes:
        name = state_codes[code]
        states.append((name, f'demand_emp_demand.aspx?lflag=eng&file1=empprov&page1=s&state_name={name}&state_code={code}&fin={year}-{year + 1}&fin_year={year}-{year + 1}&source=national&rblhpb=Both&Digest=1lod/und/+S0kFJ2NqO2ww'))

    for state, href in states:
        print('- State:', state)
        url = 'https://nreganarep.nic.in/netnrega/' + href
        #print(url)
        try:
        ...
        try:
            ...
            df = dfs[7]
            df['state'] = state
            clean_and_save_csv(df, f'{year}-csv/r5_4_district_{state}.csv', 1)
            soup2 = BeautifulSoup(r1.content, 'lxml')
            districts = []
            for l in soup2.find_all('a', {'href': re.compile('demand_emp_demand\.aspx.*')}):
            ...
            for dt in districts:
                ...
                if r3.status_code == 200:
                    try:
                        ...
                        clean_and_save_csv(df, f'{year}-csv/r5_4_block_{district}+{state}.csv', 2)
                        soup3 = BeautifulSoup(r3.content, 'lxml')
                        blocks = []
                        for l in soup3.find_all('a', {'href': re.compile('demand_emp_demand\.aspx.*')}):
                        ...
                        for bl in blocks:
                            ...
                            if r4.status_code == 200:
                                try:
                                    ...
                                    df['district'] = district
                                    df['block'] = block
                                    clean_and_save_csv(df, f'{year}-csv/r5_4_panchayat_{block}+{district}+{state}.csv', 3)
                                except Exception as e:
                                    print('ERROR', e)
                            else:
